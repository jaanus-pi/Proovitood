  ...
  return (<div>
    <div className="container">
      ...
      <Table className="table table-hover table-bordered">
        ...
        <tbody>
        {/* TODO: Order the products by price. Kas peaks ka slice() kasutama? */}
        {products
          ...
          .map(product =>
            <tr key={product.name + product.price}>
              <td>{product.name}</td>
              <td>{product.price}</td>
              {/*  TODO: Display the quantity in red if it is lower than 3. HETKEL kasutab !important Products.css failis */}
              <td className={product.quantity <= 3 && "lowQuantity"}>{product.quantity}</td> 
              <td>{product.store}</td>
            </tr>
        )}
